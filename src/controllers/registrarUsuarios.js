...
const registrarUsuarios = async (req, res) => {
  const {nombre_completo, dni, usuario, password, idperfil } = req.body;
  const estado = 1;
  ...
  try {
    const insertUsuario = `
      INSERT INTO usuarios (nombre_completo,dni,usuario, password, estado, idperfil)
      VALUES ($1, $2, $3, $4, $5, $6);
    `;
    ...
    await new Promise((resolve, reject) => {
      conectar.query(
        insertUsuario,
        [nombre_completo,dni,usuario, passwordHasheado, estado, idperfil],
        (error) => {
          if (error) {
